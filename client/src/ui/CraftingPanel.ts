import { getItem, LIFE_SKILL_LABEL, LIFE_SKILLS, RECIPES, type SelfState } from "@rox/shared";

export interface CraftingHandlers {
  onCraft(recipeId: string): void;
}

// Cooking panel (the "Cook" NPC): shows life-skill levels and lets the player
// craft any recipe they have the gathered ingredients for. Re-renders from
// each SelfSync while open.
export class CraftingPanel {
  ...
  private render(self: SelfState): void {
    ...
    this.levelsEl.innerHTML = LIFE_SKILLS.map((id) => {
    ...
    for (const recipe of Object.values(RECIPES)) {
      ...
      const canCraft = recipe.inputs.every((i) => have(i.itemId) >= i.qty);
      ...
      row.innerHTML =
        `<div class="craft-main"><div class="craft-name">${recipe.name}</div>` +
